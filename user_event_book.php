...
{
	$id=intval($_GET['del']);
	$adn=mysqli_query($conn,"delete from booking_details where b_id='$id'");
...
<html>
 ...
 <div id="cssmenu"><!--class="fs-special-view-left-menu"-->
        <ul>
          <li class="has-sub"><a href='user_profile.php' ><span class="glyphicon glyphicon-user"></span>My Profile</a>
            <ul  id="currentMenu" style="display: block">
              ...
              <li><a   class="currentsubLink" href="user_event_book.php"  class="unselected">My Bookings</a></li>
              <
            </ul>
          </li>
    ...
    <div class="rightArea">
      ...
      <div class="heading">
        <div style="float:left;width:30%;"><h2>My Bookings</h2></div>
        <!--
                -->
      </div>
      ...
      <!--Data Section Start-->
		...
		$search=mysqli_query($conn,"SELECT distinct * FROM event_details inner join booking_details where event_details.e_id=booking_details.e_id and event_details.UserID='$email'");
		...
		?>
		 ...
		 if($search){
			...
			echo"<td>$t_qty</td>";
			echo"<td>$t_price</td>";
			echo"<td><a href='user_event_book.php?edit=$eventid' title='Edit Record' onclick='return confirm('Do you want to edit?');'>Edit<i class=''></i></a></td>";
			echo"<td><a href='user_event_book.php?del=$eventid' title='Delete Record' onclick='return confirm('Do you want to delete');'>Delete<i class=''></i></a></td>";
